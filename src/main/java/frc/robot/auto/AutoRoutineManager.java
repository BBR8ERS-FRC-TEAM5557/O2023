...
public class AutoRoutineManager {
        ...
        private void generateAutoChoices() {
                ...
                m_chooser.addOption("3 Piece (F)", Commands.sequence(
                                ...
                                //elevator.tuckWaitCommand(10.0),
                                getFollowComand(m_trajectoryMap
                                                .get("LeftCubeScoreToLeftMidIntake")),
                ...
                m_chooser.addOption("2 Piece (F)", Commands.sequence(
                                new PrintCommand("2 Piece (F)"),
                                getPoseResetCommand(m_trajectoryMap.get("LeftToSweepToCubeScore")),
                                Superstructure.scoreConeLevel(NodeLevel.HIGH),
                                elevator.tuckWaitCommand(10.0),
                                getFollowComand(m_trajectoryMap.get("LeftToSweepToCubeScore")),
                ...
                m_chooser.addOption("1 Piece + Evac/Intake (F)", Commands.sequence(
                                new PrintCommand("Starting 1 Piece + 1 Pause (F)"),
                ...
                m_chooser.addOption("Score Cone + Balance (M)", Commands.sequence(
                                ...
                                Superstructure.scoreConeLevel(NodeLevel.HIGH),
                                elevator.tuckWaitCommand(5.0),
                                getFollowComand(m_trajectoryMap.get("MiddleToBalance")),
                                AutoBalance.autoBalanceCommand()));

                m_chooser.addOption("1 Piece + Evac/Intake (B)", Commands.sequence(
                                new PrintCommand("Starting 1 Piece + 1 Pause (B)"),
                ...
                m_chooser.addOption("Score Cone (A)",
                                Commands.sequence(new PrintCommand("Starting Score Cone"),
                                                getPoseResetCommand(m_trajectoryMap
                                                                .get("RightToIntake")),
                                                Superstructure.scoreConeLevel(NodeLevel.HIGH)));
        }
        ...
        private void generateTrajectories() {
                ...
                for (File file : files) {
                        ...
                        m_trajectoryMap.put(prefix, PathPlanner.loadPath(prefix,
                                        SwerveConstants.kMaxAttainableSpeed,
        ...
        private void generateEventMap() {
                ...
                m_eventMap.put("prepScoreHighCone",
                                new PrintCommand("[Scoring High Cone!!!]").andThen(Superstructure
                                                .setSuperstructureScore(() -> NodeLevel.HIGH,
                                                                () -> GamePiece.CONE)));
                m_eventMap.put("prepScoreHighCube",
                                new PrintCommand("[Scoring High Cube!!!]").andThen(Superstructure
                                                .setSuperstructureScore(() -> NodeLevel.HIGH,
                                                                () -> GamePiece.CUBE)));
        }
        ...
        private Command getFollowComand(PathPlannerTrajectory path) {
                PPSwerveControllerCommand followCommand = new PPSwerveControllerCommand(path,
                                ...
                                true, // Should the path be mirror ed depending on alliance color.
                                swerve // Requires swerve subsystem
                );

                return new FollowPathWithEvents(followCommand, path.getMarkers(), m_eventMap)
                                .alongWith(new InstantCommand(() -> Logger.getInstance()
                                                .recordOutput("PathFollowing", path)),
                                                new InstantCommand(() -> RobotStateEstimator
                                                                .getInstance().addFieldTrajectory(
                                                                                "AutoTrajectory",
                                                                                path)));
        }

        private Command getPoseResetCommand(PathPlannerTrajectory path) {
                PathPlannerState transformedState = PathPlannerTrajectory.transformStateForAlliance(
                ...
                return new InstantCommand(() -> setPose(transformedPose))
                                .andThen(homeSuperstructureCommand());
        }

        private Command homeSuperstructureCommand() {
                return Commands.parallel(RobotContainer.m_elevator.homeElevator()).withTimeout(0.75);
        }
